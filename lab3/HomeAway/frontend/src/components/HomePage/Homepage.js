...
import {HomePageSearchMutation} from '../../mutation/mutations'
import {homePageSearchQuery} from '../../queries/queries'
        ...
        else
        {
            if(this.state.location == "" || this.state.startDate == "" || this.state.endDate == "" || this.state.guests == "")
                ...
                this.props.client.query({
                    ...
                    variables : {
                        email : emailT,
                        pCity : this.state.location,
                        pAvailableStart : this.state.startDate,
                        pAvailableEnd : this.state.endDate,
                        pAccomodates : this.state.guests
                    }
                }).then(response => {
                    this.setState({
                        flag:true
                    })
                    console.log("\nThe response obtained after success in homepage search query is : ");
                    ...
                    alert("Some error in homepage search query");
                })
        ...
        if(localStorage.getItem('cookieT')=="traveler"){
            usernamedropdown = (
                <li class="dropdown items">
                    <a href="#" class="dropdown-toggle mainlinks" data-toggle="dropdown"> {localStorage.getItem('setFirstname')} <span class="caret"></span></a>
...
export default withApollo(compose(
    graphql(HomePageSearchMutation, { name: "HomePageSearchMutation" })
)(Homepage));
